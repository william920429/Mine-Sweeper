	...
	for(int x=0; x<row; x++)
		for(int y=0; y<column; y++){
			...
			if(it->status == flagged){
				if(it->isBomb) it->status = flag_with_bomb;
				else it->status = flag_with_X;
				it->update();
				
			}else if(it->status == covered){
				if(it->isBomb) it->status = bomb;
	            else it->status = game_end[it->bomb_num];
	            it->update();
				
			}else if(it->status == question){
				if(it->isBomb) it->status = question_with_bomb;
	            else it->status = game_end[it->bomb_num];
	            it->update();
...
block* mouseToArr(int _x, int _y){
    ...
    if( xout > block_size || yout > block_size) return NULL;
    else return XYtoArr(x, y);
}
				...
				if( event.button.button == SDL_BUTTON_LEFT ){
					...
					for(int i=0,j=total_bomb; i<j&&i<total_block; i++){
						...
						if(yes){
							...
							it2->isBomb = true;
							it2->bomb_num++;
							if(it2 = XYtoArr(x-1, y  ), it2!=NULL) it2->bomb_num++;
							if(it2 = XYtoArr(x+1, y  ), it2!=NULL) it2->bomb_num++;
							if(it2 = XYtoArr(x  , y-1), it2!=NULL) it2->bomb_num++;
							if(it2 = XYtoArr(x  , y+1), it2!=NULL) it2->bomb_num++;
							if(it2 = XYtoArr(x-1, y-1), it2!=NULL) it2->bomb_num++;
							if(it2 = XYtoArr(x-1, y+1), it2!=NULL) it2->bomb_num++;
							if(it2 = XYtoArr(x+1, y-1), it2!=NULL) it2->bomb_num++;
							if(it2 = XYtoArr(x+1, y+1), it2!=NULL) it2->bomb_num++;
						}
